...
class Anjuke2SQL:
    ...
    def insert_data(cls, name, price, aver, build_time, subdistrict, community, address, source, link, area, house_type, floor,
        ...
        sql = "INSERT INTO oldhouse (`name`, `price`, `aver`, `build_time`, `subdistrict`, `community`, `address`, " \
              "`source`,`link`, `area`, `house_type`, `floor`, `decoration`, `orientation`, `build_type`, `structure`," \
              " `use` ) VALUES (%(name)s, %(price)s, %(aver)s, %(build_time)s, %(subdistrict)s, %(community)s, " \
              "%(address)s, %(source)s, %(link)s, %(area)s, %(house_type)s, %(floor)s, %(decoration)s, %(orientation)s," \
              " %(build_type)s, %(structure)s, %(use)s)"

        values = {
            "name": name, "price": price, "aver": aver, "build_time": build_time, "subdistrict": subdistrict,
            "community": community, "address": address, "source": source, "link": link, "area": area,
